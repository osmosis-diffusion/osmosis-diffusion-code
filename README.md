...
To evaluate the trained RGBD prior, we subject both natural outdoor images and underwater images to the diffusion sampling process. The objective is to guide the process towards generating an RGB image similar to the input and examining the depth map generated by the prior.

Several images are available in the ```./data/check_prior/``` directory for examination.
...
```
python osmosis_sampling.py -c ./configs/check_prior_sample_config.yaml
```
